...
def main():
    ...
    print("=" * 60)
    print("DSL Parser for Real-Time File Monitoring System")
    print("=" * 60)

    # Create interpreter
    interpreter = DSLInterpreter()

    # Test rules - all should parse correctly now
    test_rules = [
        "4C = 1C + 2C + 3C",
        "51C IS ALPHANUM",
        "5C IS NUMERIC",
    ...
    for i, rule_text in enumerate(test_rules, 1):
        parsed = interpreter.parse_rule(rule_text)
    ...
    test_data = {
        ...
        2: 20,  # 2C
        3: 30,  # 3C
        4: 60,  # 4C (should be 10+20+30=60)
        5: "123.45",
        10: 5,  # 10C
        20: 99,  # 20C
        30: "ExpectedValue",  # 30C
        40: "This is a test string",  # 40C
        50: "A123",  # 50C
        51: "ABC123",  # 51C
        60: 15,  # 60C
        70: "Some value",  # 70C
    ...
    results = interpreter.validate_data(test_data)
    ...
    failing_data = {
        ...
        2: 20,
        3: 30,
        4: 50,  # Should be 60 (10+20+30)
        51: "ABC@123",  # Contains @, not alphanumeric
        30: "WrongValue",  # Doesn't match "ExpectedValue"
        60: 25,  # Outside range 10-20
        70: None,  # Missing required value
    }

    # Create new interpreter
    interpreter2 = DSLInterpreter()

    # Parse all test rules
    for rule_text in test_rules:
        interpreter2.parse_rule(rule_text)

    results = interpreter2.validate_data(failing_data)

    print("\nFailing Data Validation Results:")
    print("-" * 40)

    for i, result in enumerate(results, 1):
        status = "PASS" if result["passed"] else "FAIL"
        print(f"Rule {i}: {status} - {result['message']}")
